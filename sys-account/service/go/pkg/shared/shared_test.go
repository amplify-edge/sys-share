...
func testIsSuperadmin(t *testing.T) {
	...
	isSuper := shared.IsSuperadmin(role)
	assert.Equal(t, true, isSuper)
}

func testIsAdmin(t *testing.T) {
	role := &pkg.UserRoles{Role: pkg.ADMIN}
	isAdmin := shared.IsAdmin(role)
	assert.Equal(t, true, isAdmin)
}

func testAllowOrgMember(t *testing.T) {
	validOrg := utilities.NewID()
	roleErr := &pkg.UserRoles{Role: pkg.ADMIN}
...
func testAllowOrgAdmin(t *testing.T) {
	...
	roleValid := &pkg.UserRoles{Role: pkg.ADMIN, OrgID: validOrg}
	allowed, err = shared.AllowOrgAdmin(&pkg.Account{
		Id:    utilities.NewID(),
		Email: "someemail@example.com",
		Role:  roleValid,
...
func testAllowProjectMember(t *testing.T) {
	...
	t.Log("with wrong project id")
	roleValid2 := &pkg.UserRoles{Role: pkg.USER, OrgID: validOrg, ProjectID: validProject}
	allowed, err = shared.AllowProjectMember(&pkg.Account{
		Id:    utilities.NewID(),
		Email: "someemail@example.com",
		Role:  roleValid2,
